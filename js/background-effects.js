/**
 * Ram editz — Background layers (deferred until page is interactive)
 */
(function () {
  'use strict';
  ...
  function enhance() {
    if (mount.dataset.bgEnhanced) return;
    ...
    var particleCount = isLite ? 0 : (isSmooth ? 2 : 4);
    var particles = '';
  ...
  if (document.body.classList.contains('is-loading')) {
    window.addEventListener('rameditz:page-loaded', enhance, { once: true });
  } else if ('requestIdleCallback' in window) {
    requestIdleCallback(enhance, { timeout: 800 });
  } else {
