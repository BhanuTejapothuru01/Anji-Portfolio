...
(function () {
  ...
  var STORAGE_KEY = 'ram-editz-feedback-muted';
  ...
  var HAPTIC = {
    light: [18],
    medium: [35, 12, 35],
    menuOpen: [50, 15, 65, 15, 85, 20, 100],
    menuClose: [60, 25, 45],
    nav: [40, 12, 40],
    play: [35, 10, 55, 10, 80],
    success: [50, 30, 50, 30, 90],
    filter: [28, 10, 38],
    slider: [15],
    reveal: [12, 8, 12],
    focus: [10]
  ...
  function isMuted() {
    return muted || window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  }
  ...
  var SOUNDS = {
    ...
    filter: function () {
      ...
      vibrate(HAPTIC.filter);
    },
    slider: function () {
      tone(280 + Math.random() * 80, 0.03, 'sine', 0.04);
      vibrate(HAPTIC.slider);
    },
    hover: function () {
  ...
  function play(name, force) {
    ...
    var wasMuted = muted;
    if (force) muted = false;
    try { SOUNDS[name](); } catch (e) { /* audio blocked */ }
    if (force) muted = wasMuted;
  }
  ...
  function toggleMute() {
    ...
    updateToggleUI();
    play(wasMuted ? 'muteOff' : 'muteOn', true);
    return muted;
  }
  ...
  function injectToggle() {
    ...
    btn.type = 'button';
    btn.setAttribute('aria-label', 'Toggle sound and haptics');
    btn.innerHTML =
  ...
  function bindGlobal() {
    ...
    document.addEventListener('click', function (e) {
      ...
      if (el.classList.contains('logo')) { play('logo'); return; }
      if (el.classList.contains('nav-link')) { play('nav'); return; }
      if (el.classList.contains('home-back-btn')) { play('nav'); return; }
      if (el.classList.contains('marquee-item')) { play('marquee'); return; }
      if (el.closest('.footer-col')) { play('footer'); return; }
      if (el.classList.contains('service-card') || el.closest('.service-card')) { play('service'); return; }
      if (el.classList.contains('testimonial-card') || el.closest('.testimonial-card')) { play('testimonial'); return; }
      if (el.closest('.video-thumb') || el.closest('.featured-reel') || el.closest('.hero-reel') || el.classList.contains('play-btn')) { play('play'); return; }
      if (el.classList.contains('filter-btn')) { play('filter'); return; }
      if (el.classList.contains('btn-submit')) return;
      if (el.classList.contains('btn-primary') || el.classList.contains('btn-service') || el.classList.contains('btn-outline')) { play('click'); return; }
