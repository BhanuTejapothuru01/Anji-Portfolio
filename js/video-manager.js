...
(function () {
  ...
  function getStreamUrl(url) {
    ...
    var slow =
      conn &&
      (conn.saveData ||
        /2g|3g|slow-2g/.test(conn.effectiveType || ''));
  ...
  function buildThumbHtml(video) {
    ...
    if (!hasUrl) {
      inner +=
        '<span class="video-placeholder">ADD URL IN js/videos.js</span>';
    } else if (!thumb) {
      inner +=
        '<span class="video-placeholder">YOUR VIDEO HERE</span>';
    }
  ...
  function buildVideoCard(video, index) {
    ...
    return (
      '<article class="video-card reveal" data-video-id="' +
      escapeHtml(
      ...
      ((thumb ||
        (hasUrl && isDirectVideo(video.url)))
        ? ' has-thumbnail'
        : '') +
      (hasUrl ? '' : ' is-pending') +
      '"' +
  ...
  function buildFeaturedHtml(featured) {
    ...
    var hasBgVideo =
      hasUrl && isDirectVideo(featured.url);
    ...
    var reelClass =
      'featured-reel featured-reel-hud reveal' +
      (hasBgVideo
        ? ' has-bg-video'
        : thumb
        ? ' has-thumbnail'
    ...
    return (
      ...
      '<div class="featured-hud-rec">' +
      '<span class="featured-rec-dot"></span> REC' +
      '</div>' +
      ...
      (!hasUrl
        ? '<p class="featured-hint">Add your showreel URL in js/videos.js</p>'
        : '') +
  ...
  function initFeaturedTimecode() {
    ...
    timecodeInterval = setInterval(function () {
      ...
      document
        .querySelectorAll('[data-timecode]')
        .forEach(function (el) {
          el.textContent = text;
        });
  ...
  function buildHeroReelHtml(featured) {
    ...
    return (
      ...
      '<div class="hero-reel-corners" aria-hidden="true">' +
      '<span></span><span></span><span></span><span></span>' +
      '</div>' +

      '<div class="hero-reel-rec">' +
      '<span class="hero-rec-dot"></span> REC' +
      '</div>' +
  ...
  function renderHeroReel() {
    ...
    if (!featured) {
      mount.innerHTML =
        '<div class="hero-reel hero-reel-placeholder">Add your showreel in js/videos.js</div>';

      return;
    }

    mount.innerHTML =
      buildHeroReelHtml(featured);
  ...
  function initBackgroundVideos() {
    ...
    window.addEventListener(
      ...
      function () {
        ...
        bgVideos.forEach(function (
          videoEl
        ) {
          if (
            !isPrimaryReel(videoEl)
          ) {
            return;
          }
    ...
    if (
      !initBackgroundVideos.globalBound
    ) {
      initBackgroundVideos.globalBound =
        true;
  ...
  /* ==========================================================
     HOME PAGE — NEW VIDEOS ONLY
     ==========================================================
     
     These are the 3 videos that should appear in:
     
     HOME PAGE
     → Stories that resonate
     
     Portfolio still contains ALL videos.
  */
  ...
  function renderHomePreview() {
    ...
    /*
     * ONLY THESE THREE NEW VIDEOS
     * are displayed on the HOME PAGE.
     *
     * The old portfolio videos remain untouched.
     */
    ...
    var videos =
      allVideos.filter(function (
        video
      ) {
        return (
          homeVideoIds.indexOf(
            video.id
          ) !== -1
        );
      });
    ...
    /*
     * Keep the order exactly as specified
     * above, regardless of their position
     * inside videos.js.
     */

    videos.sort(function (a, b) {
      return (
        homeVideoIds.indexOf(a.id) -
        homeVideoIds.indexOf(b.id)
      );
    });

    if (videos.length === 0) {
      mount.innerHTML =
        '<p class="featured-empty">Add the new videos in <code>js/videos.js</code></p>';
      ...
      videos
        .map(function (
          ...
          var cardInner =
            ...
            '<span class="featured-card-num">0' +
            (i + 1) +
            '</span>' +
  ...
  function openModal(
    ...
    modalEls.modal.classList.add(
      'open'
    );

    modalEls.modal.setAttribute(
      'aria-hidden',
      'false'
    );
  ...
  function closeModal(
    ...
    modalEls.modal.classList.remove(
      'open'
    );
    ...
    modalEls.modal.removeAttribute(
      'aria-modal'
    );
